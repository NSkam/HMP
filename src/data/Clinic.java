...
public class Clinic {
    ...
    private ArrayList<Application>Application_list = new ArrayList<Application>(); //lista aithsewn
    ...
    public boolean checkOnCallDate(String ocdate1, String ocdate2){ //orismata -> 2 hmrnies os string
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/mm/yyyy", Locale.ENGLISH); //formatter gia date auths ths morfhs
        LocalDate d1 = LocalDate.parse(ocdate1, formatter); //metatrepoyme apo string se date simfona me to formatter
        LocalDate d2 = LocalDate.parse(ocdate2, formatter);
        
        if ( d2.isBefore(d1) || d2.isAfter(d1)) //an i 2h imerominia einai prin h meta thn prwth 
            return true;
        else
            return false;
    }
    ...
    public void updateOnCallList(String cdate, int amka){
        ...
        OnCall oc = new OnCall(cdate,amka, name, j+1); //neo antikeimeno efhmerias 
